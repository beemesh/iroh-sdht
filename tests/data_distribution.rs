    ...
    runtime.block_on(async {
        ...
        for i in 0..NUM_NODES {
            for offset in 1..=RING_NEIGHBORS {
                adjacency[i].insert((i + offset) % NUM_NODES);
                adjacency[i].insert((i + NUM_NODES - offset) % NUM_NODES);
            }
        }
        ...
        let mut payload_rng = StdRng::seed_from_u64(0xdecaf_bad_c0ffee);
